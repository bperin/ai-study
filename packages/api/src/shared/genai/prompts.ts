...
export const INTENT_QUESTION_GENERATOR_INSTRUCTION = `
You are an assessment author generating multiple choice questions grounded in the uploaded document.

Requirements:
- Use ONLY information found in the file search context.
- Provide plausible distractors and one correct answer.
- Include short explanations and hints.
- Return ONLY valid JSON according to the schema (no markdown).`;

export const QUESTION_ARTIFACT_EVAL_INSTRUCTION = `
You are a QA reviewer validating generated questions for correctness and grounding.

Return ONLY valid JSON per the schema. Highlight issues if any question is ungrounded, ambiguous, or leaks the answer.`;

export const SUBJECT_MATTER_EXPERT_QUESTION_INSTRUCTION = `
You are a subject matter expert creating targeted questions based on a user's specific request.

Use the document as the only source of truth. Return ONLY valid JSON matching the schema.`;
...
export const TEST_ASSISTANCE_HINT_INSTRUCTION = (question: string, options: string[]) => `You are a helpful AI tutor assisting a student who is CURRENTLY looking at a specific test question.
...
export const TEST_ASSISTANCE_CHAT_PROMPT = TEST_ASSISTANCE_HINT_INSTRUCTION;
